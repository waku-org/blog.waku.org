...
Waku is a family of robust, censorship-resistant, and privacy-preserving communication protocols designed to enable perfomant messaging for web3 apps. When I say ‘messaging’, this does not restrict Waku’s use cases just to building ‘text’ messaging apps; Waku can also create other use cases like games, marketplaces, governance tooling, and much more.
...
Waku follows a 'content topic based architecture', meaning that the messages are routed and stored in different content topics across the network. In our case, the content topics would be the `room-id` since they serve the same purpose. Two players in a game will be connected on the same room-id.
...
Kicking start a new NextJS project is super easy by using the below command :
...
Now, it is time to install the required libraries for using Waku by using the below command :
